...
n_detectors = e1_config.e1_n_detectors2().astype('int')
...
for ss_id, ss in enumerate(subspace_sizes):
    for drf_type in drf_types:
        ...
        np.save('results_ex1/2_clf_%s' % str_name, results_clf)
        np.save('results_ex1/2_drf_arr_%s' % str_name, results_drf_arrs)
pbar.close()