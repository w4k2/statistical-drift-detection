...
results_all = np.zeros((replications, len(streams), len(
    drf_types), len(drifts_n), len(detector_names), 4))
...
for str_id, stream in enumerate(streams):
    ...
    for d_id, d in enumerate(drifts_n):
        for d_type_id, d_type in enumerate(drf_types):
            ...
            res_arr = np.load(
                'results_ex2_real/drf_arr_str_%s.csv_%s_%idrfs_all.npy' % (stream, d_type, d))
            print(res_arr.shape)  # reps, detectors, chunks-1

            res_clf = np.load(
                'results_ex2_real/clf_str_%s.csv_%s_%idrfs_all.npy' % (stream, d_type, d))
            ...
            #accuracy
            results_all[:, str_id, d_type_id, d_id,
                        :, 0] = np.mean(res_clf, axis=2)
            #d1
            results_all[:, str_id, d_type_id,
...
for str_id, stream in enumerate(streams):
    for drf_id, drf_type in enumerate(drf_types):
        ...
        # combination of drifts number and feature number
