...
for rec_id, rec in enumerate(recurring):
    for drf_id, drf_type in enumerate(drf_types):
        res_clf = np.load('results_ex2_d_f/clf_10feat_1drifts_%s_%s.npy' %(drf_type, rec))
        # replications x detectors x chunks-1
        res_clf_mean = np.mean(res_clf, axis=0)

        res_arr = np.load('results_ex2_d_f/drf_arr_10feat_1drifts_%s_%s.npy' %(drf_type, rec))
